...
- Question：
  ...
  2. **每一层的常见协议？**
     - 应用层：HTTP、FTP（文件传输）、SMTP（电子邮件）、SNMTP（网络管理）、TELNET（远程终端介入）、DNS
     - 传输层：TCP、UDP
...
- Question：
  ...
  3. **GET/POST区别？**
     ...
     - | 区别                                           | GET                                                          | POST                                               |
       ...
       | 缓存                                           | 能被缓存                                                     | 不可以被缓存                                       |
       | 传参方式                                       | 使用URL或者Cookie传参                                        | 将数据放在BODY中                                   |
       | 幂等(多次访问结果相同)安全(不会破坏服务器资源) | 幂等且安全                                                   | 不幂等不安全                                       |
  ...
  12. **HTTPS优缺点？**
      ...
      - 优点：混合加密解决窃听，摘要算法解决篡改，CA证书解决冒充，大幅增加了中间人攻击的成本
      - 缺点：安全是有范围的，技术门槛高，CA证书收费，需要更多的服务器资源
  ...
  20. **SST/TLS握手过程 /HTTPS工作过程/HTTPS原理？**
      ...
      - 四次握手：
      - 第一次（Client Hello）：客户端向服务端发起连接请求，客户端发一个随机数
      - 第二次（Server Hello）：服务器把公钥给客户端，服务端发一个随机数，服务器自己保留私钥
      - 第三次（Client Key Exchange）：客户端用公钥对秘钥（客户端随机数 + 服务端随机数）加密，发给服务端
...
- Question：
  ...
  3. **TCP/UDP对应的场景？**
     ...
     - TCP：面向连接的，可靠的；FTP文件传输；HTTP/HTTPS（SMTP、HTTP、FTP、TELNET）
     - UDP：面向无连接，不可靠的；DNS、SNMP；音视频、直播；广播通信(DNS、SNMP)
  ...
  8. **TCP如何保证的可靠性？**
     ...
     - 校验和：检验接受的数据是否有异常和差错，如果出现差错，则丢弃TCP段，重发
  ...
  9. **TCP保活机制/保活计时器？**（keeplive）
     ...
     - 只要收到对端数据，计时器就复位重新计时
     - [TCP和HTTP中keepalive的区别](https://www.jianshu.com/p/9fe2c140fa52)
  ...
  13. **流量控制？**
      ...
      - 流量控制：每次接收到数据后会将剩余可处理数据的大小告诉发送方
  ...
  16. **粘包、拆包？**
      ...
      - 粘包：发送端的几个数据包小于TCP缓冲区大小；接收端的应用层没有及时读取接收缓冲区中的数据
      - 拆包：发送端的一个数据包大于TCP缓冲区大小；大于MSS大小；大于MTU大小
      - 解决：添加首部（发送端给每个数据包添加首部，首部中包含数据包的长度，接收端可以知道每个数据包的实际长度）；设置固定长度（发送端将每个数据包设置固定长度，接收端读取固定长度数据把数据包分开）；增加边界（数据包之间增加边界，回车、空格等特殊字符，接收端可以通关这个特殊字符区分）
