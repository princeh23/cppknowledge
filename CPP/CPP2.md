...
- Conception：
  ...
  15. #define 
  16. typedef
  17. const 指针常量、常量指针 顶层const、底层const
  18. volatile、mutable、explicit
  19. inline
  20. #ifdef、#else、#endif、#ifndef
  21. #program once
  22. extern"C"
  23. ++i、i++
  24. **函数相关**
  25. sizeof、strlen
  26. strcpy、sprintf、memcpy
  27. 
  28. **其他**
  29. 异常处理（try catch throw exception类）
...
- Question：
  ...
  9. **指针相关（常量指针/指针常量、悬挂指针/野指针、指针数组/数组指针、函数指针）**
...
- Conception：
  ...
  13. 析构函数
  14. 
  15. 虚函数、纯虚函数、虚函数表（virtual table）、虚函数指针
  16. 
  17. 继承 组合
  18. 
  19. 重载、隐藏、重写/覆盖
...
- Question：
  ...
  14. **虚函数**
      ...
      - [讲解](https://blog.csdn.net/qq_31597573/article/details/51438996)
  ...
  24. **强制类型转换符？**（竖着看）
      ...
      | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ---- |
      | 通常用于**数值类型**、**向上**转换                           | 通常用于**多态类型**转换；**借助RTTI进行类型转换，基类中包含虚函数**（RTTI中存在一个继承链，查找能否找到要转换的类型） | **删除const、volatile**属性（只能改变底层const） | 各种之间的转换。二进制位的重新解释                           | dynamic_cast指针转换失败返回目标类型的nullptr；对于**引用**转换失败引发**bad_cast**异常 |      |
      | **向上转换安全**（子类->父类），**向下转换不安全**（父类->子类） | 向上向下转换均可，向上转型始终安全（因为基类有虚函数），向下时有类型检查功能 |                                                  | 指针转换为任意指针；整数转换为指针/引用；指针/引用转换为整数 |                                                              |      |
      |                                                              | 指向派生类的基类指针转换为派生类的指针或引用； Child *pchild = dynamic_cast<Child *>(pfather); | 删除的不是变量的特性，删除的是指针/引用的常量性  | 不安全（更好的办法是使用类的set/get函数）                    |                                                              |      |
      | 转换失败：编译期间转换，转换失败抛出**编译错误**             | 转换失败：**指针**转换失败返回**NULL**；**引用**转换失败抛出**bad_cast**异常 |                                                  |                                                              |                                                              |      |
...
- Question：
  ...
  1. **new/delete和malloc/free？**
     ...
     - |                            | new/delete                                                   | malloc/free                    |
       ...
       | **分配内存的大小**         | 编译器计算                                                   | 指定大小                       |
       | 函数重载                   | 允许（[全局new有6种重载形式](https://www.cnblogs.com/lemaden/p/10150665.html)） | 不允许（C中的库函数）          |
       | 是否可以相互**调用**       | operator new的实现可以基于malloc，delete实现可以用free       | malloc的实现不可以调用new      |
  ...
  1. **字节对齐、结构体对齐？**
     ...
     - 为什么需要进行内存对齐：操作系统不是一字节一字节读取，而是一段一段进行读取（如果int是1-4字节存储，按字节读取的化需要分开读取再组合，效率不高）
...
- Question：
  ...
  14. **vector中push_back的时间复杂度分析？**

      - n个元素，倍增因子为m
      - 重新分配的次数大概是logm(n)，第i次分配需要复制m^i个元素
      - n次push_back花费的总时间约为nm/(m - 1)
      - ∑1 ~ logm(n)  m^i = nm/(m - 1)
      - m/(m - 1)是一个常数
      - 均摊到每次的时间复杂度为O(1)
      - [参考](http://blog.sina.com.cn/s/blog_a2a6dd380102w73e.html)
  ...
  20. **为何map和set的插入删除效率比其他序列容器高，而且每次insert之后，以前保存的iterator不会失效？**
  ...
  24. **unordered_map、unordered_set、unordered_multimap、unordered_multiset底层原理？**
  ...
  31. **为什么要用deque作为stack和queue的底层默认容器？**
  
      - stack需要push_back和pop_back()，list、vector、deque均可
      - 为什么不用vector：
  ...
  33. **sort函数源码？**
      ...
      - 数据量很大使用**快速排序**
      - 递归过程中，分段之后数据量很小，使用**插入排序**（数据大致有序时候为O(n)，快排取元素存在不确定性，快排在数据本身有序的时候是最慢的O(n^2)）
      - 递归过程中，递归层次过深，使用**堆排序**处理（递归层数多浪费时间，堆排序最好最坏都是nlogn，归并也是胆需要递归）
      - [讲解](https://blog.csdn.net/wzh1378008099/article/details/108173812)
...
- Question：
  1. **C++11新特性？**
     ...
     - 多线程：thread、atomic、variable（线程休眠）、lock_guard、mutex、condition_variable（条件变量）、sem_t（信号量）
     - 函数：function、bind
  ...
  3. **右值引用、移动构造、完美转发**
     ...
     左值：等号左边，可以取地址
     右值：等号右边，不可以取地址
     左值持久，右值短暂，右值只能绑定到临时对象，右值引用可以接管引用对象的内容
  ...
  4. **lock_guard、mutex**
     ...
     - lock_guard：
       - 原因：手动上锁解锁比较复杂
       - lock_guard</mutex> lock(g_mutex);自动进行加锁，离开作用域时候进行解锁
...
- Question：
  ...
  1. 如果程序崩溃的话，怎么使用GDB调试？

     - [参考](https://blog.csdn.net/tekenuo/article/details/88394780)

     - ```
       ./a.test
       run 执行文件（可以看到崩溃的提示信息）
       where 查看可能出错的位置
       list 查看附近代码
       break 设置断点
       run 重新运行程序
  ...
  2. **GDB命令相关**
     ...
     - frame n（没有n查看**当前**栈帧的信息，有n打印指定栈帧）
     - up、down（向上向下移动，栈帧间切换）
     - info frame（当前栈的信息）
     - info args（打印参数信息）
...
- Question：
  ...
  4. **memcpy()、memmove()**
     ...
     - ![img](https://pic3.zhimg.com/80/v2-c5725ef19e0b7cde0c9b4b9bfa0b50e2_720w.jpg)
     ...
     void *memcpy(void *dest, const void *src, size_t n){
         ...
         //情况1包括在了不重叠的情况种
         //覆盖情况2
         if(s_src < s_dest && s_dest + size > s_src){
             //指向了字符串末尾
             s_dest = s_dest + n - 1;
             s_src = s_src + n - 1;
             //从后往前
             while(n--) *s_dest-- = *s_src--;
         }else{
             //从前往后
             while(n--) *s_dest++ = *s_src++;
         }
