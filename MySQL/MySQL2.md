...
- Question：
  ...
  1. **说说MVCC？有什么作用？**
     ...
     - 多版本并发控制
     - 作用：处理读写冲突的手段，提高高并发场景下的吞吐能力；读操作会看到一致性的快照，实现非阻塞的读，用于支持提交读和可重复读的实现
  ...
  1. 可重复读解决幻读了吗？（细解决）
     ...
     - 解决了读数据的幻读，没解决修改数据的幻读(update)，要解决就要MVCC + 间隙锁[讲解](https://blog.csdn.net/godloveleo9527/article/details/109554263#%E4%BA%8C%E5%8F%AF%E9%87%8D%E5%A4%8D%E8%AF%BB%E9%9A%94%E7%A6%BB%E7%BA%A7%E5%88%AB%E8%83%BD%E8%A7%A3%E5%86%B3%E5%B9%BB%E8%AF%BB)
     - Next-Key锁实现
...
- Question：
  ...
  2. **bin log是什么？redo log是什么？undo log是什么？relay log是什么？**
     ...
     - | redo log                       | undo log                         |
       | ------------------------------ | -------------------------------- |
       | 保证事务原子性和持久性         | 保证一致性                       |
       | 先将事务的所有操作写入redo log | 根据undo log恢复到更改之前的状态 |
