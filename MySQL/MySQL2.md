...
- Question：
  ...
  4. **关系型数据库和非关系型数据库？**
     ...
     - 关系型数据库：
       ...
       - 缺点：并发量高；每天产生的数据多，查询效率低；难以进行横向扩展，升级扩展时候需要进行停机维护和数据迁移
       - MySQL、Oracle
     - 非关系型数据库：
       - 非关系型、分布式的，不保证ACID；以键值对进行存储，结构不固定，可以根据需要增加键值对，减少时间空间的开销
       - 优点：根据键值对就可以完成查询，不需要进行多表联合查询；
...
- Question：
  1. InnoDB和MyISAM区别
     ...
     | 文件组成 | frm：表定义文件；idb：数据文件（数据文件和索引在一块）       | frm：表定义文件；myd：数据文件；myi：索引文件                |        |
     | 锁       | 表、行锁                                                     | 表锁                                                         |        |
...
- Question：
  ...
  1. **可重复读解决幻读了吗？**
     ...
     - 解决了**读数据**的幻读，没解决**修改数据**的幻读(update)，要解决就要串行化/**MVCC + next-key锁**
     - 
  ...
  1. **什么是MVCC？**
     ...
     - 用于支持**提交读**和**可重复读**的实现
     - **READ VIEW**和**数据中的隐藏列**进行比对，决定是否可读
     - ![图片](https://mmbiz.qpic.cn/mmbiz_png/J0g14CUwaZcteHSfStxxCZ9XJia9z0VQ6zcicwk676KAJiaPewibNyZNV0OreQicYbnu8o7ZsZpbZkibJ8ribiaaMEP5Jg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
     -  事物的READ VIEW                 聚簇索引的隐藏字段
  ...
  1. **Read View**
     ...
     - MVCC内部使用的**一致性读快照**/读视图称为**Read View**
     ...
     - **RC**隔离级别：**每次读取数据前**，都生成一个readview；
     - **RR**隔离级别：**在事务开启前**，生成一个readview；
...
- Question：
  1. **锁的分类？**
     ...
     - [讲解](https://www.jianshu.com/p/d2ac26ca6525)
  ...
  1. **行级锁？**
     ...
     - 行级锁加锁规则（小林coding）
       ...
       - Next-key Lock：默认加的是临键锁，部分情况下回进行退化，加的是一个前开后闭的区,
       - eg：4，8，16三条数据，检索10时候锁(8,16]，
...
- Question：
  ...
  4. **分表算法？**
     ...
     - 范围方式
     - 一致性hash（哈希环）
     - [讲解](https://www.cnblogs.com/alunchen/p/11614060.html)
  5. **分库分表中间件？**
